...
class PaginationDetector:
    ...
    process_page_job_listing_with_pagination: Callable
    ...
    def extract_xpaths_from_container(
    ...
    def identify_pagination_container(
        ...
        def count_base_links(tag: Tag) -> int:
            if not base_url:
                return 0 
            hrefs = [
                a.get("href", "") for a in tag.find_all("a") if isinstance(a, Tag)
            ]

            return sum(
                base_url
                in (href if isinstance(href, str) else " ".join(href or []))
                for href in hrefs
            )
        ...
        for index, tag in enumerate(unique_candidates):
            ...
            for _ in range(max_retries):
                ...
                try:
                    ...
                    try:
                        validated = ContainerIdentifier.model_validate(result_structured)
                    except Exception as e:
                    ...
                    xpaths_info = self.extract_xpaths_from_container(
    ...
    def extract_pagination_buttons(
        self, soup: BeautifulSoup, base_url: str
    ) -> PaginationButtons:
        """
        ...
        if known_containers:
            ...
            for container_html in self.containers_html[base_url]:
                pagination_data = self.extract_xpaths_from_container(
                    container_html, soup
                )
        ...
        container_identified = self.identify_pagination_container(soup, base_url)
        ...
        pagination_data = self.extract_xpaths_from_container(container_html, soup)

        return pagination_data

    def check_if_pagination_buttons(self, url: str, retries=1) -> List[str]:
        """
        ...
        try:
            ...
            result_pagination_buttons = self.extract_pagination_buttons(soup, url)
            ...
            if retries > 0:
                ...
                return self.check_if_pagination_buttons(url, retries=retries - 1)
            else:
    ...
    def handle_dynamic_pagination(
        ...
        try:
            ...
            self.process_page_job_listing_with_pagination(
                url, base_url, dynamic_pagination=True
            )
    ...
    def handle_standard_pagination(
        self, button_xpath: str, url: str, base_url: str
    ) -> None:
        ...
        self.process_page_job_listing_with_pagination(
            new_url, base_url, dynamic_pagination=False
        )
