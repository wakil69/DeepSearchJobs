...
from worker_async.core.jobs_scraper import EmailJobsScraper
from worker_async.core.job_listings_scraper import FetchJobsListingsScraper
from worker_async.core.website_scraper import WebsiteScraper
    ...
    try:
        with psycopg.connect(**connection_params) as conn:
            with conn.cursor() as cur:
                ...
                job_urls: set[str] = {r[0] for r in cur.fetchall() if r[0]}
...
async def process_analyser_job(
    ...
    if not company.get("website"):
        websiteScraper = WebsiteScraper(
            company_id, company_name, job_logger, browser, ch
        )
    ...
    redis_client.hset(job_key, mapping={"status": "done", "retries": retries, "job_listings_step_done": "false"})
