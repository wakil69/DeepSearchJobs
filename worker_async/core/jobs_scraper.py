...
from worker_async.base_scraper import BaseScraper
from worker_async.core.prompts import (
    PROMPT_EXTRACT_JOBS,
)
from worker_async.core.llm_utils import call_llm_structured
from worker_async.core.show_more_button_detector import ShowMoreButtonDetector
from worker_async.core.pagination_detector import PaginationDetector
from worker_async.core.post_process_jobs import PostProcessingJobs
...
class EmailJobsScraper(BaseScraper):
    ...
    async def process_page_job_listing_with_pagination(
        ...
        for attempt in range(retries + 1):
            try:
                ...
                text_content = self.extract_structured_text(soup, url, self.job_offers)
                ...
                pagination_data = await self.pagination_detector.extract_pagination_buttons(
                    soup, base_url
                )
        ...
        for button_xpath in pagination_buttons:
            try:
                if "[@href=" in button_xpath:
                    await self.pagination_detector.handle_standard_pagination(
    ...
    async def extract_job_listings(self, job_pages: List[str]) -> None:
        ...
        for i, job_page in enumerate(job_pages, 1):
            ...
            try:
                ...
                pagination_buttons = await self.pagination_detector.check_if_pagination_buttons(job_page)
            ...
            with conn.cursor() as cur:
                ...
                for job in jobs:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Skipped job due to error: {job_error} — {job}"
                        )
    ...
    def update_old_jobs(self, conn) -> None:
        ...
        if not self.old_job_offers:
            self.logger.info("No old jobs to update.")
            return
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    RETURNING id
                    """,
                    (self.company_id, self.old_job_offers),
                )
                ...
                self.logger.info(
                    f"Updated {len(updated_job_ids)} old job opportunities to is_existing = FALSE"
                )
